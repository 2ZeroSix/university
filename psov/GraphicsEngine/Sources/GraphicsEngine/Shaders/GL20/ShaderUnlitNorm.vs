...
void main()
{
	//gl_Position = matrixWorldViewProjT * vec4(radius * normilize(position), 1.0);	// OpenGL
	gl_Position = (vec4(radius * normilize(position), 1.0) + vec4(position, 1.0)) / 2 * matrixWorldViewProjT;	// DirectX
